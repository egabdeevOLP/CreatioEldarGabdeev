	...
	return {
		...
		methods: {
			onStageItemClick: function(stageItem) {
				var prevStage = this.getMasterEntityParameterValue("Stage");
			...
			validateMaxToursNumber: function(callback, scope) {
				...
				const esq = scope.Ext.create("Terrasoft.EntitySchemaQuery", {rootSchemaName: "Opportunity"});
				
				esq.addColumn("ResponsibleDepartment");

				esq.filters.addItem(
					scope.Terrasoft.createColumnFilterWithParameter(
						scope.Terrasoft.ComparisonType.EQUAL, "Id", id));

				esq.getEntityCollection(function(response) {
					var result = {success: true};
					if (response.success && response.collection.getCount() >= 0) {
						if(!Ext.isEmpty(response.collection.collection.items[0].$ResponsibleDepartment)){
							result.message = "1234";
